...
encuesta$Q10_Estrato[!is.na(encuesta$Q24_area) & is.na(encuesta$V51_Avance)] <- NA
...
plot(estrato1$Q08_PAPA, main = "Estrato 1 vs PAPA", xlab = "Número de encuesta", ylab = "PAPA", ylim = c(3,5))
grid(NA,22)

plot(estrato2$Q08_PAPA, main = "Estrato 2 vs PAPA", xlab = "Número de encuesta", ylab = "PAPA", ylim = c(3,5))
grid(NA,22)

plot(estrato3$Q08_PAPA, main = "Estrato 3 vs PAPA", xlab = "Número de encuesta", ylab = "PAPA", ylim = c(3,5))
grid(NA,22)

plot(estrato4$Q08_PAPA, main = "Estrato 4 vs PAPA", xlab = "Número de encuesta", ylab = "PAPA", ylim = c(3,5))
grid(NA,22)

plot(estrato5$Q08_PAPA, main = "Estrato 5 vs PAPA", xlab = "Número de encuesta", ylab = "PAPA", ylim = c(3,5))
grid(NA,22)
```
...
***¿Tienen mejor PAPA aquellos estudiantes que son de estrato socioeconómico alto y que residen en el AMVA en comparación con aquellos que son de estrato socioeconómico bajo y habitan por fuera de esta?***

Se podría esperar que una persona que vive en el Área Metropolitana del Valle de Aburrá con buenos ingresos económicos, tenga más posibilidades de mejorar su promedio académico, ya que se encuentra más cerca de la Universidad, y tiene más posibilidades de asesorías privadas o cursos para mejorar en su promedio, mientras que alguien fuera del Valle de aburrá con pocos ingresos le es mucho más díficil adquirir estos privilegios.
...
Seguramente el género no influya mucho, pero pensamos que mientras más conspiraciones falsas el estudiante haya considerado como verdaderas, es más propenso a tener un PAPA bajo, ya que consideramos que las personas que respondieron las conspiraciones correctamente cuentan con un espirítu de investigación que tal vez una persona que la respondió incorrectamente no tenga.

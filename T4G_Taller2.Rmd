...
table(intervalos_PAPA, encuesta$numero_conspiraciones, dnn = c("Intervalos del PAPA","# Conspiraciones"))
```
...
***¿Son los estudiantes con mayor avance en la carrera los que respondieron mejor las preguntas de álgebra y geometría?***

Inicialmente se cree que aquellos estudiantes que tengan un mayor avance en su carrera hayan respondido correctamente las preguntas sobre el área sombreada y los divisores; en otras palabras, se piensa que aquellos que tengan un menor avance en su carreras habrán respondido incorrectamente dichas preguntas. Recordar que la respuesta correcta del área sombreada es ***"4*pi"***, además que solamente quienes hallan seleccionado las tres opciones en la pregunta de los divisores habrán respondido correctamente en su totalidad esa pregunta.

#### Resumen numérico

```{r, echo = FALSE}
#respuestas_correctas <- encuesta$Q24_area[encuesta$Q24_area == "4*pi"]
...
table(intervalos_avance, encuesta$Q24_area, dnn = c("Intervalos de avance en la carrera","Selección área sombreada"))
```

```{r, echo = FALSE}
intervalos_avance <- cut(encuesta$V51_Avance,c(10,20,30,40,50,60,70,80),include.lowest=TRUE)

table(intervalos_avance,encuesta$opciones_divisor, dnn = c("Intervalos de avance en la carrera","Número de opciones"))
```

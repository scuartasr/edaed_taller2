...
```{r, echo = FALSE}
plot(intervalos_PAPAhom, enc_hombres$Edad, main = "PAPA vs Edad (Hombres)", xlab = "Intervalo del PAPA", ylab = "Edad (Años)", ylim = c(17,39))
grid(NA,24)

plot(intervalos_PAPAmuj, enc_mujeres$Edad, main = "PAPA vs Edad (Mujeres)", xlab = "Intervalo del PAPA", ylab = "Edad (Años)", ylim = c(17,39))
grid(NA,24)
```
...
Pensamos que tal vez las personas que más se destacan en promedio y puntaje en sus pruebas saber, tienen en alta estima al resto del curso, y consideran que el grupo tuvo un buen desempeño en las preguntas de álgebra y geometría.
...
# Resumen numerico, personas que si residen en el Area Metropolitana del Valle de Aburra


Se realizara una tabla donde se evidencia el numero de personas que estan residiendo en el area metropolitana y que pertenecen a los estratos 1, 2, 3, 4, y 5.
...
```
Resumen numerico arrojado

```{r, echo = FALSE}}
summary(estratodentro$Q08_PAPA)
```
```{r}
summary(estratodentro$Q08_PAPA)
```
...
# Resumen numerico, personas que no residen en el Area Metropolitana del Valle de Aburra
...
```{r}
summary(estratofuera4$Q08_PAPA)
```
```{r}
summary(estratofuera$Q08_PAPA)
```

```{r, , echo = FALSE, fig.height=5, fig.width=5}
plot(enc_aream$Q10_Estrato, enc_aream$Q08_PAPA,
     main = "Estrato VS P.A.P.A - Vivir en Area Metropolitana",
     xlab = "Estrato Socioeconomico",
     ylab = "PAPA")

plot(enc_noaream$Q10_Estrato, enc_noaream$Q08_PAPA,
     main = "Estrato VS P.A.P.A - No vivir en Area Metropolitana",
     xlab = "Estrato socioeconomico",
     ylab = "PAPA")
```
